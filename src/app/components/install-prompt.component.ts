...
@Component({
  ...
  standalone: true,
  imports: [],
  template: ` @if (isIos() && !isRunningStandalone()) {
      <div
        id="ios-toast-default"
        class="flex items-center w-full p-4 text-gray-500 bg-white shadow"
        role="alert"
      >
        <div class="ms-3 text-sm font-normal">
          Tap the menu button and then select "Add to Home Screen" to install
          Pokemon PWA.
        </div>
        <button
          type="button"
          class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8"
          data-dismiss-target="#ios-toast-default"
          aria-label="Close"
        >
    ...
    @if (isAndroid() && shouldInstall()) {
      <div
        id="android-toast-message-cta"
        class="w-full p-4 text-gray-500 bg-white shadow dark:bg-gray-800 dark:text-gray-400"
        role="alert"
      >
        <div class="flex gap-4">
          <div class="ms-3 text-sm font-normal">
            <span
              ...
              >Add Pokemon PWA to homescreen.</span
            >
          </div>
          <button
            type="button"
          ...
          <button
            ...
            data-dismiss-target="#android-toast-message-cta"
            aria-label="Close"
          >
            <span class="sr-only">Close</span>
            <svg
...
export class InstallPromptComponent implements OnInit, OnDestroy {
  ...
  private readonly beforeInstallPromptEventText = 'beforeinstallprompt';
  ...
  ngOnDestroy(): void {
    ...
    window.removeEventListener(
      this.beforeInstallPromptEventText,
      (event: any) => {
