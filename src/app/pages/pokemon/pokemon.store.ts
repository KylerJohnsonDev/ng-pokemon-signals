  ...
  withMethods((state, pokemonService = inject(PokemonService)) => ({
    loadPokemonByIdentifier: rxMethod<string | number>(
      pipe(
        ...
        switchMap((identifier) => pokemonService.loadPokemonByIdentifier(identifier).pipe(
          tap((pokemon) => patchState(state, { pokemon })),
          switchMap((pokemon) => {
            const types = pokemon.types.map((type) => type.type.name);
            const sources = types.map((type) => pokemonService.loadTypeInfo(type));
            return forkJoin(sources).pipe(
              map((typeInfoCollection: TypeInformation[]) => {
                const { goodAgainst, badAgainst } = getUniqueGoodAgainstBadAgainstTypes(typeInfoCollection)
                patchState(state, { goodAgainst, badAgainst, isLoading: false})
              })
            )
          }),
        )),
    ...
    resetState: () => patchState(state, initialPokemonState),
  })),
  withHooks({
    onInit(store) {
